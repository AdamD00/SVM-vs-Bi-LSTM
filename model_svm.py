...
def train_and_evaluate_svm(language, data_folder):
    ...
    # max_features ogranicza słownik do 10 000 najważniejszych słów (przyspiesza SVM i zapobiega przeuczeniu)
    tfidf = TfidfVectorizer(max_features=10000)

    X_train_vec = tfidf.fit_transform(X_train_text)
    # Dla zbioru testowego używamy TYLKO transform (nie fit!), aby model nie "oszukiwał"
    X_test_vec = tfidf.transform(X_test_text)

    # 3. Trening modelu SVM
    print("Trwa trening modelu SVM (to może zająć od kilku sekund do kilku minut)...")
    # kernel='linear' to standard i najlepszy wybór dla klasyfikacji tekstu
    ...
    # ==============================================================================
    # NOWY BLOK: ANALIZA BŁĘDÓW (ERROR ANALYSIS)
    # ==============================================================================
    ...
    # Zapisujemy po 50 losowych przykładów do plików CSV, żebyś mógł je wygodnie przeczytać
